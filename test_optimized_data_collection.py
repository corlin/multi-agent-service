...
async def test_chinese_keyword_mapping():
    ...
    try:
        ...
        from multi_agent_service.patent.models.requests import PatentDataCollectionRequest
...
async def test_data_collection_request_conversion():
    """测试请求转换功能."""
    logger.info("\n=== 测试请求转换功能 ===")
    ...
    try:
        ...
        from multi_agent_service.config.agent_config import AgentConfig
...
async def main():
    ...
    # 测试2: 请求转换
    test2_success = await test_data_collection_request_conversion()
    
    # 测试3: Agent能力
    test3_success = await test_agent_capabilities()
    
    # 总结
    logger.info("\n" + "=" * 60)
    logger.info("📊 测试总结:")
    logger.info(f"  中文关键词映射: {'✓ 成功' if test1_success else '✗ 失败'}")
    logger.info(f"  请求转换功能: {'✓ 成功' if test2_success else '✗ 失败'}")
    logger.info(f"  Agent能力测试: {'✓ 成功' if test3_success else '✗ 失败'}")
